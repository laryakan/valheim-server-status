...
- Go into the service menu with option **20**
  ...
  - add and activate vss.http.service (server status over HTTP) with option **7** (without sudo, the setup will tell you how to do it)
    - be sure to me sudoer or root when executing this option, and be sure no service with that name already exist
...
- setup vss.http.service in **./setup** option **20** then **7** (only if you want your server status displaying over HTTP)
